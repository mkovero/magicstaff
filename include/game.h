...
static positionSample quaternion_to_euler_ypr(float x, float y, float z, float w)
{
...
inline static int map_yaw_circular_deg(float yaw_deg, float offset_deg)
{
...
inline static float minimal_angle_diff_deg(float a, float b)
{
...
inline static uint8_t scale_to_8bit(float value, float rng, bool use_abs)
{
    float v;
...
static colorSample gyroColor(float values[3])
{
    positionSample pos;
    colorSample color;
    float qx = values[0];
