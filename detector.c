...
static const char *classify_single_sample(float ax_val, float ay_val)
{
    ...
    if (x_magnitude > y_magnitude * 1.2f) {  // X-axis dominant
        if (ax_val > 0)
            return "right";
        else
            return "left";
    } else if (y_magnitude > x_magnitude * 1.2f) {  // Y-axis dominant
        if (ay_val > 0)
            return "up";
        else
            return "down";
...
static const char *classify_gesture(float *ax, float *ay, int start, int end)
{
    if (end <= start)
        return "unknown";
    ...
    if (x_dominance > y_dominance * 1.2f) {  // X-axis dominant (with some tolerance)
        if (ax_mean > 0)
            return "right";
        else
            return "left";
    } else if (y_dominance > x_dominance * 1.2f) {  // Y-axis dominant
        if (ay_mean > 0)
            return "up";
        else
            return "down";
...
void detectorTask(void *params)
{
    ...
    for (;;)
    {
        if (xQueueReceive(sensorQueue, &buf, portMAX_DELAY) == pdPASS)
        {
            ...
            if (buf->head >= 2)
            {
                ...
                for (int i = 1; i < buf->head; i++)
                {
                    ...
                    if (dt_ms > 0 && dt_ms < 1000) { // reasonable sample interval (1ms to 1s)
            ...
            for (int i = last_index; i < buf->head; i++)
            {
                ...
                if (latency >= 0 && latency < 1000) { // reasonable processing delay (< 1s)
            ...
            if (buf->head >= 2) {
                ...
                printf("Current sample ts: %ld, Previous sample ts: %ld\n", curReceived, prevReceived);
                ...
                else
                {
                    ...
                    if (gesture_active)
                    {
                        ...
                        if (gesture_above_count >= MIN_LEN)
                        {
                            ...
                            const char *direction = classify_gesture(gesture_ax_history, gesture_ay_history, 0, gesture_history_count);
                            printf("🎯 GESTURE COMPLETE: %s (duration=%d samples, samples=%d)\n", 
            ...
            if (gesture_active && gesture_above_count >= MIN_LEN)
            {
                ...
                const char *direction = classify_gesture(gesture_ax_history, gesture_ay_history, 0, gesture_history_count);
                printf("🎯 GESTURE COMPLETE (end of batch): %s (duration=%d samples)\n", 
