...
void add_sample(SensorBuffer *buf, uint8_t type, int64_t timestamp, float values[VECTOR_SIZE])
{
    ...
    { // > 1e12 - likely nanoseconds
        // Nanoseconds since epoch - convert to milliseconds since epoch
        s->timestamp = timestamp / 1000000;
    }
    else if (timestamp > 1000000000LL)
    { // > 1e9 - milliseconds since epoch
...
void handle_json(const char *json, int length, SensorBuffer *accelBuffer)
{
    ...
    {
        gameSample.timestamp = timestamp;
        gameSample.received_ms = get_current_ms();
        for (int i = 0; i < VECTOR_SIZE; i++)
        {
            gameSample.values[i] = values[i];
        }
        if (gameQueue != NULL)
        {
            xQueueSend(gameQueue, &gameSample, portMAX_DELAY);
        }
        else
        {
            printf("gameQueue is NULL, send failed\n");
        }
    }
    else if (strcmp(type, "android.sensor.linear_acceleration") == 0)
    {
        add_sample(a, LINEAR_ACCEL, timestamp, values);
...
void netprocess(void *pvParameters)
{
    ...
    SensorBuffer *accelBuffer = pvPortMalloc(sizeof(SensorBuffer));
    accelBuffer->head = 0;
    while (accelQueue == NULL)
    {
        vTaskDelay(pdMS_TO_TICKS(1000));
    };
    xQueueSend(accelQueue, &accelBuffer, portMAX_DELAY);
    ...
    retry_recv:
        ...
        if (n < 0)
        {
            ...
            goto retry_recv;                    // interrupted, try again
            perror("recvfrom failed");
            continue;
        }
        buffer[n] = '\0';
        handle_json(buffer, n, accelBuffer);
    }
